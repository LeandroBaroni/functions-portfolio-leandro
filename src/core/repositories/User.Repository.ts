...
import { AddDocument, FirebaseWhere, ReadOptions, SetDocument, UpdateDocument, WriteOptions } from '@typings/typings';
...
import { OrderByDirection, Query, SetOptions, WhereFilterOp } from 'firebase-admin/firestore';
...
export class UserRepository {
  ...
  constructor(private readonly firebaseRepository: FirebaseRepository) {
    ...
    const collectionPath = FirestoreCollectionName.USERS;
   ...
   */
  public async add(data: AddDocument<User>, options: WriteOptions = { timestamps: true }) {
   ...
   */
  public async update(data: UpdateDocument<User>, options: WriteOptions = { timestamps: true }): Promise<void> {
    const clone = toFirestore(data);
   ...
   */
  public async set(data: SetDocument<User>, options: SetOptions & WriteOptions = { timestamps: true }): Promise<void> {
    const clone = toFirestore(data);
   ...
   */
  public async getById(id: string, options: ReadOptions = { timestamps: true }): Promise<User> {
    const doc = await this.collection.doc(id).get();

    if (!doc.exists) {
      throw new DocumentNotFoundError(FirestoreCollectionName.USERS, id);
    }

    return ofFirestore<User>(doc, options.timestamps);
  }
   ...
   */
  public async getByIds(ids: string[], options: ReadOptions = { timestamps: true }): Promise<User[]> {
   ...
   */
  protected getWhereMany(
    filters: FirebaseWhere<User>[],
    limit: number | null = null,
    orderBy: keyof User | null = null,
    orderByDirection: OrderByDirection | null = null,
    options: ReadOptions = { timestamps: true }
   ...
   * ], 'lastLogin', 'desc');
   */
  protected async getOneWhereMany(
    filters: FirebaseWhere<User>[],
    orderBy: keyof User | null = null,
    orderByDirection: OrderByDirection | null = null,
    options: ReadOptions = { timestamps: true }
   ...
   * @param {ReadOptions} [options={ timestamps: true }] - Opções de leitura.
   * @returns {Promise<User[]>} Array de documentos resultantes da consulta.
   */
  protected async getDocs(query: Query, options: ReadOptions = { timestamps: true }): Promise<User[]> {
    const { docs } = await query.get();

    return docs.map((document) => ofFirestore<User>(document, options.timestamps));
  }
}
